...
%       timeUpdateMatrix   - n x k+1 matrix showing which nodes were allowed to update every step



%   Based on EVOLVECRBN() and EVOLVEDGARBN() by Christian Schwarzer - SSC EPFL (20.01.2003).
%   Author: Masado Ishii - ECE PSU
%   CreationDate: 03.01.2018 LastModified: 03.01.2018
...
% For every node that is cued on step i, we'll light its TUM entry for step i+1. (M.Ishii 08.01.2018)
...
for i=2:k+1
    ...
    % Find which nodes get updated by Cascade Update. (M.Ishii 08.01.2018)
    % I.e. those nodes which were "cued" on the last time step.
    cuedVector = logical([nodeUpdated.cued]);
    ...
    % Record which nodes were allowed to update. (M.Ishii 08.01.2018)
    % Note, cuedVector reflects which nodes were
    % cued on the last time step, same as which were
    % actually updated on this time step.
    timeUpdateMatrix(:,i) = cuedVector';
